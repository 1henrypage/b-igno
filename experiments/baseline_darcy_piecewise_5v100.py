...
# %% [markdown]
# # Baseline Posterior Inference: Darcy Piecewise {5, 100}
#
...
for SEED in SEEDS:
    ...
    FIGURE_DIR = Path(f'figures/baseline_darcy_piecewise_5v100/seed_{SEED}')
    ...
    print(f"a_true range: [{float(a_true.min()):.1f}, {float(a_true.max()):.1f}] (expect {{5, 100}})")

    # ### Prior Predictive Baseline

    a_true_np = np.array(a_true[0, :, 0])
    def _prior_icorr(pred, true):
        pred_thresh = np.where(np.array(pred) >= 52.5, 100.0, 5.0)
        return float(cross_correlation(jnp.array(pred_thresh), jnp.array(true), k_low=5.0, k_high=100.0))
    ...
    icorr_map = cross_correlation(a_map, a_true[0], k_low=5.0, k_high=100.0)
    ...
    pw_metrics = compute_piecewise_metrics(a_samples_np, a_true_np, 5.0, 100.0, a_err_fn=lambda pred, true: cross_correlation(pred, true, k_low=5.0, k_high=100.0))
    ...
    plot_metrics_table({
        ...
        'sigma': SIGMA,
    }, title='Darcy Piecewise 5v100 (20x contrast) — RQ1 Metrics')
    ...
    }, title='RQ1 — Bootstrap CI on Posterior Metrics (Darcy Piecewise 5v100)')

    chain_metrics = compute_per_chain_metrics(a_samples_np, a_true_np, NUM_CHAINS, k_low=5.0, k_high=100.0, a_err_fn=lambda pred, true: cross_correlation(pred, true, k_low=5.0, k_high=100.0))
    print_per_chain_table(chain_metrics, NUM_CHAINS)

    # ### Metric Convergence

    conv_result = compute_metric_convergence(a_samples_np, a_true_np, k_low=5.0, k_high=100.0, a_err_fn=lambda pred, true: cross_correlation(pred, true, k_low=5.0, k_high=100.0))
...
# %%
print_cross_seed_summary("baseline", "darcy_piecewise_5v100")
