...
# %% [markdown]
# # Out-of-Distribution Robustness: Darcy Piecewise
#
...
IN_DATA_PATH = '../data/darcy_piecewise/pwc_test_in.mat'
OOD_DATA_PATH = '../data/darcy_piecewise/pwc_test_out.mat'
...
PROBLEM_NAME = 'ood_darcy_piecewise'
...
def log_pde_fn(beta, rho_pde):
    ...
    a_vals = a_sample(a_prob[..., None])  # rng=None -> deterministic
...
def _decode_piecewise(beta_samples, x_full):
    ...
    for i in range(0, n_s, DECODE_BATCH):
        ...
        a_prob = jax.nn.sigmoid(a_logits)
        a_decoded = a_sample(a_prob[..., None])  # (bs, n_points, 1)
...
print(f"In-domain a_true range: [{float(a_true_in_tune.min()):.1f}, {float(a_true_in_tune.max()):.1f}] (expect {{5, 10}})")
...
dec_baseline = compute_piecewise_metrics(a_np_bl, a_true_in_tune_np, 5.0, 10.0, a_err_fn=cross_correlation)
...
for rho in RHO_PDE_VALUES:
    ...
    a_np_s = a_pred_s[:, :, 0]
    dec = compute_piecewise_metrics(a_np_s, a_true_in_tune_np, 5.0, 10.0, a_err_fn=cross_correlation)
...
def run_condition(model_fn, model_kwargs, x_full, a_true_ref, obs_indices,
    ...
    metrics = compute_piecewise_metrics(a_samples, a_true_np, 5.0, 10.0, a_err_fn=cross_correlation)
...
for SEED in SEEDS:
    ...
    icorr_map_in = float(cross_correlation(a_map_in, a_true_in[0]))
    ...
    icorr_map_ood = float(cross_correlation(a_map_ood, a_true_ood[0]))
    print(f"[seed={SEED}] OOD MAP I_corr: {icorr_map_ood:.6f}")

    # === Prior Predictive Baseline ===
    _icorr_fn = lambda pred, true: float(cross_correlation(
        jnp.array(np.where(np.asarray(pred) >= 7.5, 10.0, 5.0)), jnp.array(true)
    ))
    ...
    plot_metrics_comparison_table_4way(
        ...
        title=f'Darcy Piecewise — sigma={SIGMA}, rho_pde={BEST_RHO_PDE} (seed={SEED})',
    )
    ...
    }, title=f'Darcy Piecewise — RQ3: Pairwise Contrasts (Bootstrap, seed={SEED})')
    ...
    }, title=f'Darcy Piecewise — Physics Benefit Analysis (seed={SEED})')
    ...
    plot_posterior_gallery(
        ...
        piecewise_a_bounds=(5.0, 10.0), grid_shape=(29, 29),
        a_true=a_true_ood_np, n_show=6,
...
# %%
print_cross_seed_summary("ood", "darcy_piecewise")
