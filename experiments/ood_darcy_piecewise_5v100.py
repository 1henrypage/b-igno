...
# %% [markdown]
# # Out-of-Distribution Robustness: Darcy Piecewise {5, 100}
#
...
from src.problems.darcy_piecewise import (
    DarcyPiecewise5v100 as DarcyPiecewise,
    mollifier,
    a_sample,
...
IN_DATA_PATH = '../data/darcy_piecewise_5v100/pwc_test_in.mat'
OOD_DATA_PATH = '../data/darcy_piecewise_5v100/pwc_test_out.mat'
...
PROBLEM_NAME = 'ood_darcy_piecewise_5v100'
...
print(f"In-domain a_true range: [{float(a_true_in_tune.min()):.1f}, {float(a_true_in_tune.max()):.1f}] (expect {{5, 100}})")
...
dec_baseline = compute_piecewise_metrics(a_np_bl, a_true_in_tune_np, 5.0, 100.0, a_err_fn=lambda pred, true: cross_correlation(pred, true, k_low=5.0, k_high=100.0))
...
for rho in RHO_PDE_VALUES:
    ...
    a_np_s = a_pred_s[:, :, 0]
    dec = compute_piecewise_metrics(a_np_s, a_true_in_tune_np, 5.0, 100.0, a_err_fn=lambda pred, true: cross_correlation(pred, true, k_low=5.0, k_high=100.0))
...
def run_condition(model_fn, model_kwargs, x_full, a_true_ref, obs_indices,
    ...
    metrics = compute_piecewise_metrics(a_samples, a_true_np, 5.0, 100.0, a_err_fn=lambda pred, true: cross_correlation(pred, true, k_low=5.0, k_high=100.0))
...
for SEED in SEEDS:
    ...
    icorr_map_in = float(cross_correlation(a_map_in, a_true_in[0], k_low=5.0, k_high=100.0))
    ...
    icorr_map_ood = float(cross_correlation(a_map_ood, a_true_ood[0], k_low=5.0, k_high=100.0))
    print(f"[seed={SEED}] OOD MAP I_corr: {icorr_map_ood:.6f}")

    # === Prior Predictive Baseline ===
    _icorr_fn = lambda pred, true: float(cross_correlation(
        jnp.array(np.where(np.asarray(pred) >= 52.5, 100.0, 5.0)), jnp.array(true),
        k_low=5.0, k_high=100.0,
    ))
    ...
    plot_metrics_comparison_table_4way(
        ...
        title=f'Darcy Piecewise 5v100 — sigma={SIGMA}, rho_pde={BEST_RHO_PDE} (seed={SEED})',
    )
    ...
    }, title=f'Darcy Piecewise 5v100 — RQ3: Pairwise Contrasts (Bootstrap, seed={SEED})')
    ...
    }, title=f'Darcy Piecewise 5v100 — Physics Benefit Analysis (seed={SEED})')
    ...
    plot_posterior_gallery(
        ...
        piecewise_a_bounds=(5.0, 100.0), grid_shape=(29, 29),
        a_true=a_true_ood_np, n_show=6,
...
# %%
print_cross_seed_summary("ood", "darcy_piecewise_5v100")
